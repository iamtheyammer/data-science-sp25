...
- Are there any sources of *erroneous* uncertainty in the percent
  ...
  - Yes, when counting the number of houses that are unoccupied, I could
    have missed one. I do know, however, that all unoccupied houses I
    counted are actually unoccupied, as when I clicked on them, they say
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - I would argue that it is not - this dataset just includes the first
    25 houses, and people with the same last name (possibly families)
    could live closer together. A random sample may be more
    representative of all houses in Helvig.
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - Random
  - A random sample removes factors like who moved in first (possibly a
    big family), families who live near each other, etc.
...
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - They could be families and families often want to live nearby each
    other.
...
```
    ...
    ## 1 Blomgren 0.0172    0.0714  0.138   0.05 percentile

**Observations**:

- What is highest possible prevalence for your chosen name, based on the
  confidence interval you constructed?
  - Blomgren’s highest possible prevalence is 0.138, nearly double its
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - Our data is not representative of the entirety of Helvig, therefore
    we can get an idea of how wrong we are but not actually become
    correct (or closer to the actual value).
...
- (What steps will you take to collect the data?)
  ...
  - To start, we will be taking a random sample of **20 households from
    ...
    3.  Following the same protocol as used in challenge eight to
        generate the sample 
  - From each parent in each household, we will collect. For straight
...
- (How will you choose your sample size?)
  - We chose our sample size based on the number of cities and the
    number of households that it’s reasonable for each person to collect
    data on. 20 households times 6 cities creates a total sample size of
    120 households. 
