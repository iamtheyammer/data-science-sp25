...
- How many aluminum alloys are in this dataset? How do you know?
  ...
  ``` r
  df_stang_long %>% select(alloy) %>% distinct
  ```
  ...
  - Only one: above, I looked for all the unique values in the `alloy`
    column and found only one.

- What angles were tested?

  ``` r
  df_stang_long %>% select(angle) %>% distinct
  ```
  ...
  - 0˚, 45˚, and 90˚ were tested. I looked for all the unique values in
    the `angle` column and found those three.

- What thicknesses were tested?

  ``` r
  df_stang_long %>% select(thick) %>% distinct
  ```
  ...
  - Four thicknesses were tested: 0.022, 0.032, 0.064, and 0.081 inches
    (units are stated in referenced paper).

- How much does elasticity (`E`) vary throughout the data?

  ``` r
  df_stang_long %>% select(E) %>% summary
  ```
...
df_stang_long %>%
  ...
  geom_point(size = 3) +
  #geom_smooth(method = "lm", se = FALSE) + # to help visualize, I added trendlines
  #facet_wrap(~ angle) + # and broke it up by angle to see if that matters (it doesn't, really)
  theme_minimal()
```
...
- Does this graph support or contradict the claim above?
  - The graph contradicts the claim above. As the thickness of the
    material increases, the elasticity decreases. There are some
