...
df_q3 <- 
  df_q1 %>% mutate(in_A = stat(x, y)) %>%
    summarize(count_total = n(), count_A = sum(in_A), pi_est = mean(in_A))
...
``` r
# I don't know what's wrong here... I do have the column pi_est...
```
    ...
    pi_est = map_dbl(
      ...
      function(split_df) {
        analysis(split_df) %>% 
          ...
          mutate(in_A = stat(x, y)) %>%
            summarize(pi_est = mean(in_A)) %>%
          pull(pi_est)
      }
    )
  )
...
``` r
df_q1 %>% mutate(indicator = as.numeric(x^2 + y^2 <= 1)) %>%
  summarise(
    pi_est = 4 * mean(indicator),
    se = sqrt(16 * mean(indicator) * (1 - mean(indicator)) / n)
  ) %>%
  mutate(
    lower = pi_est - 1.96 * se,
    upper = pi_est + 1.96 * se
  )
```
...
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - (Good enough as a rough estimate?) Maybe very rough? 5% is fairly
    off.
  - (Good enough as a precise estimate?) Definitely not.
- What would be a *valid* way to make your CI more narrow?
  - Increase the sample size, likely by orders of magnitude.
