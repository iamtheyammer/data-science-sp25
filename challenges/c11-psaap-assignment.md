...
df_psaap %>%
  group_by(idx) %>%
  ggplot(aes(x = x, y = T_norm, color = idx, group = idx)) +
  geom_point() +
  geom_line()
```
...
baseline_test <- df_train %>% mutate(pred = predict(fit_baseline, df_train))
cheat_test <- df_train %>% mutate(pred = predict(fit_cheat, df_train))
nonphysical_test <- df_train %>% mutate(pred = predict(fit_nonphysical, df_train))

metrics(baseline_test, truth = T_norm, estimate = pred)
```

    ## # A tibble: 3 × 3
    ##   .metric .estimator .estimate
    ##   <chr>   <chr>          <dbl>
    ## 1 rmse    standard       0.389
    ## 2 rsq     standard       0.348
    ## 3 mae     standard       0.311

``` r
metrics(cheat_test, truth = T_norm, estimate = pred)
```

    ## # A tibble: 3 × 3
    ##   .metric .estimator .estimate
    ##   <chr>   <chr>          <dbl>
    ## 1 rmse    standard       0.228
    ## 2 rsq     standard       0.777
    ## 3 mae     standard       0.189

``` r
metrics(nonphysical_test, truth = T_norm, estimate = pred)
```
    ...
    ## 2 rsq     standard     0.00130
    ## 3 mae     standard     0.388

**Observations**:

- Which model is *most accurate*? Which is *least accurate*?
  - The most accurate model is the cheat model, and the least accurate
    model is the nonphysical one, which links the value of the
    normalized fluid temperature rise to the index of the test
    performed.
...
- Would we have access to `avg_T` if we were trying to predict a *new*
  ...
  - No. If we are trying to predict temperature, we can’t use
    temperature to predict it.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - Index is a metadata variable, it has no physical meaning. It’s just
    the number of the test in the sequence.
...
- Which columns are excluded in the model formula above? What categories
  ...
  - idx
    - Category: Metadata
    - Good to leave out because: It’s not an input or output variable,
      it’s just a piece of data about the order the trial was conducted
      in.
  - avg_q, avg_T and rms_T
    - Category: Output
    - Good to leave out because: this is an output variable, and
      therefore cannot be used to predict the output.
...
- Note that literally *all* of the inputs above have *some* effect on
  ...
  - It’s important to look at the p values to get a concrete idea of how
    correlated the variables are.
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - I can tell because, if every prediction was exactly accurate, they
    would be exactly on the line `y=x` (which is shown in grey here).
  - Therefore, since the q4 points appear closer to `y=x`, those are
...
- How many predictors does the `fit_simple` model need in order to make
  ...
  - `fit_simple` used only one (`T_norm`). `fit_q4` used every input
    variable (all variables minus output and metadata variables) to
    predict the output.
